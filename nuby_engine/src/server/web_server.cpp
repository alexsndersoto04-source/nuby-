...
std::string WebServer::handle_request(const std::string& method, const std::string& path,
    ...
    if (method == "GET" && path == "/frame.seq") {
        if (shell_->tick_blink()) frame_seq_++;
        if (shell_->dirty()) { frame_seq_++; shell_->clear_dirty(); }
        std::string j = "{\"seq\":" + std::to_string(frame_seq_.load()) + "}";
        return http_response(200, "OK", "application/json", j);
    }

    if (method == "GET" && path == "/frame.raw") {
        auto& fb = shell_->frame();
        if (fb.empty()) {
    ...
    if (method == "GET" && path == "/api/stats") {
        ...
        j << "{\"documents\":" << shell_->index().document_count()
          << ",\"terms\":" << shell_->index().term_count()
          << ",\"history_entries\":" << shell_->history().size() << "}";
...
void WebServer::handle_client(int client_sock) {
    ...
    std::string resp = handle_request(method, clean_path, query, body, is_binary);
