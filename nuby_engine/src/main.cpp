...
int main(int argc, char* argv[]) {
    ...
    // Índice ÚNICO compartido por todas las sesiones (multi-usuario real)
    size_t loaded = server.index().load(data_path);
    auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(
                  std::chrono::steady_clock::now() - t0).count();
    server.set_data_path(data_path);

    std::cout << "[indice] " << loaded << " documentos reales cargados de " << data_path
              << " en " << ms << " ms\n"
              << "[indice] " << server.index().term_count()
              << " terminos unicos en el indice invertido\n" << std::flush;
    // Ya no hace falta reload_home(): cada sesión construye su home AL
    // crearse, cuando el índice ya está cargado.
